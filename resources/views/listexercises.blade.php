                    ...
                    <div class="col-sm-8">
                        ...
                        <div style="width:30%; margin-right:30px; float: right; ;margin-top:20px;" class="editworkout">
                            <form action="{{url($nexturl)}}" method="POST">
                                {!! csrf_field() !!}
                                <input type="hidden" name="publishable" id="publishable" class="form-control" value="no"/>
                                <button type="submit" id="btnNext" class="next-but">Next</button>
                            </form>

                            <button type="button" id="btnBack" class="back-but" onclick="window.location='{{ url($backurl) }}'">Back</button>
                        </div>
                    </div>
                </div>
                <!--split-->
    ...
    <div class="modal fade" id="SetModal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
        <div class="modal-dialog" >
            <div class="modal-content" style="background-color: #f3f1f5">
                ...
                <h3 class="registerModalTitleDescription" id="linemodeldescription"><strong>View Set Info</strong></h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="row" style="margin-top: 30px">
                        <div style="text-align: center">
                            <button id="modalSave" type="submit" style="width: 50%; height: 50px; background-color: #8dcea5;color: white; font-weight: bold" class="btn btn-default">OK</button>
                        </div>
                    </div>
                    </form>
                </div>
            ...
            fields: [
                ...
                {name: "SetCount", title:"Set Count", align:"center", type:"number", width: 30, validate: { message: "Set count can be 1~50", validator: "range", param: [1, 50] }},
                {
                    ...
                    itemTemplate: function(value, item) {
                        ...
                        if(setdata == "")
                        {
                            console.log("not publish able");
                            $("#publishable").val("yes");
                            if(daytype == 1)
                            {
                                return $("<a style=' color: #ff2e1f'>").text("No data");
                            }
                            else
                            {
                                return $("<div></div>");
                        ...
                        else
                        {
                            ...
                            if(daytype == 1)
                            {
                                return $("<button style='color: #345ef8'>").text("View")
                                        .on("click", function() {
            ...
            for ( var i=0; i<setscount; i++) {
                ...
                if(exerciseType == 1)
                {
                    var tag = $("<div><label class='setNoLabel'>" +
                            ...
                            "<input type=" + typeString + " id='measuvalue" + index + "' name='measuvalue" + index + "'class='form-control setinfoedit' value = '" + displayValue + "' readonly>" +
                            "<label>Reps</label>" +
                            "<input type='number' min='1' id='repsvalue" + index + "' name='repsvalue" + index + "'class='form-control setinfoedit' placeholder='Reps' value = '" + displayReps + "' readonly>" +
